...
// CustomerTypeMongoDBDao - Type DAO Repository
...
func (p *CustomerTypeMongoDBDao) InitializeDao(client utils.Map, businessId string, customerId string) {
	log.Println("Initialize Type Mongodb DAO")
	p.client = client
...
// Get - Get by code
func (t *CustomerTypeMongoDBDao) Get(TypeId string) (utils.Map, error) {
	// Get a single document
	var result utils.Map

	log.Println("CustomerTypeMongoDBDao::Get:: Begin ", TypeId)

	collection, ctx, err := mongo_utils.GetMongoDbCollection(t.client, sales_common.DbCustomerTypes)
	log.Println("Get:: Got Collection ")

	filter := bson.D{{Key: sales_common.FLD_CUSTOMER_TYPE_ID, Value: TypeId}, {}}
...
func (p *CustomerTypeMongoDBDao) Find(filter string) (utils.Map, error) {
	...
	log.Println("TypeDBDao::Find:: Begin ", filter)
	...
	log.Println("TypeDBDao::Find:: End Found a single document: \n", err)
	return result, nil
}
...
func (t *CustomerTypeMongoDBDao) Create(indata utils.Map) (utils.Map, error) {
	...
	log.Println("Type Save - Begin", indata)
	//Sales Type
	collection, ctx, err := mongo_utils.GetMongoDbCollection(t.client, sales_common.DbCustomerTypes)
	if err != nil {
...
func (t *CustomerTypeMongoDBDao) Update(TypeId string, indata utils.Map) (utils.Map, error) {
	...
	filterType := bson.D{{Key: sales_common.FLD_CUSTOMER_TYPE_ID, Value: TypeId}}
	updateResult1, err := collection.UpdateOne(ctx, filterType, bson.D{{Key: "$set", Value: indata}})
	if err != nil {
		return utils.Map{}, err
	}
...
func (t *CustomerTypeMongoDBDao) Delete(TypeId string) (int64, error) {

	log.Println("CustomerTypeMongoDBDao::Delete - Begin ", TypeId)
	...
	optsType := options.Delete().SetCollation(&options.Collation{
	...
	filterType := bson.D{{Key: sales_common.FLD_CUSTOMER_TYPE_ID, Value: TypeId}}
	resType, err := collection.DeleteOne(ctx, filterType, optsType)
	if err != nil {
		log.Println("Error in delete ", err)
		return 0, err
	}
	log.Printf("CustomerTypeMongoDBDao::Delete - End deleted %v documents\n", resType.DeletedCount)
	return resType.DeletedCount, nil
}
